...
{
	//send264.SetRTPParam("182.118.27.25",8080);
	send264.SetRTPParam("172.18.12.67",8000);
	send264.InitSocket();
	...
	//char capture_name[80] = "Logitech HD Webcam C270";
	char capture_name[80] = "Logitech HD Webcam C525";
	...
	HANDLE  hThread;
	 FILE *fp_out;
	const char* out_path = "rtmp://live.hkstv.hk.lxdns.com/live/hkszys11";	
	///const char* out_path = "rtmp://tllive.tianlaikge.com/dlroom/438405_0_111222333";	
	...
	//capture_name="sdfsdf";
	//string str="Logitech HD Webacm C270";
	//capture_name=str.c_str();
	sprintf(device_name, "video=%s", capture_name);
	ifmt=av_find_input_format("dshow");

	char filename_out[]="测试.h264"; 
	//Output bitstream  
	...
	while (av_read_frame(ifmt_ctx, dec_pkt) >= 0){	
		...
		if (dec_got_frame){
			...
			if (enc_got_frame == 1){
				...
				/*if(aa)
				{
					fwrite(enc_pkt.data, 1, enc_pkt.size, fp_out);
					send264.SendRTPH2641(enc_pkt.data,enc_pkt.size);
					aa=false;
				}	  
				if(enc_pkt.size<=1400)
				{
					fwrite(enc_pkt.data, 1, enc_pkt.size, fp_out);
					send264.SendRTPH2641(enc_pkt.data,enc_pkt.size);
				}*/
				//打包
				m_RtpPack.SetH264RtpPack(enc_pkt.data, enc_pkt.size);
				fwrite(enc_pkt.data, 1, enc_pkt.size, fp_out);
				send264.SendRTPH2641(enc_pkt.data,enc_pkt.size);
				//Write PTS
				...
				//flvsend.write_av_data(enc_pkt.data, enc_pkt.size,enc_pkt.pts , false); 
...
int flush_encoder(AVFormatContext *ifmt_ctx, AVFormatContext *ofmt_ctx, unsigned int stream_index, int framecnt){
	...
	while (1) {
		...
		//采集编码 264 并给rtplib
		//send264.SendRTPH264(enc_pkt.data,enc_pkt.size);
